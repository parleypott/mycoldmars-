// Locks the world-slug routing in api/qss-stories.js — the story-save / duplicate
// path that persists world_slug for EVERY QSS story.
//
// History: a hardcoded `if (slug === 'burgundy') return 'burgundy'; return
// 'queen-scarlet';` allowlist landmine was killed in three sibling endpoints
// (qss-worlds.js, qss-explorer.js, qss-cast.js). A FOURTH copy survived here —
// the most load-bearing one, since it decides which world's library every saved
// story lands in. This test pins the fix: qss-stories.js must route through the
// SHARED registry-driven sanitizeSlug so a future 3rd world is honored, not
// silently collapsed to queen-scarlet.
//
...
// qss-worlds / qss-explorer / qss-cast locks did. A source-binding assert catches
// any regression to a divergent local copy directly.
...
const SRC = readFileSync(join(__dirname, 'qss-stories.js'), 'utf8');
...
// The old, divergent, hardcoded copy that used to live in qss-stories.js.
...
// (so swapping in the shared resolver changed NOTHING for the live 2-world site).
const realInputs = [
...
// ---- 3. Unknown / malformed slugs fall back to the default (column can't be poisoned).
...
// If/when a 3rd world ('emerald') is added to WORLDS, the shared resolver routes a
// story saved under it to 'emerald' (correct library). The OLD hardcoded copy would
// have collapsed it to queen-scarlet — the story silently lands in the WRONG world's
// library AND assertWorldSlugPersisted can't catch it (sent==stored==queen-scarlet).
...
check('source: imports shared sanitizeSlug from qss-worlds', () => {
  assert.ok(/import\s*\{[^}]*\bsanitizeSlug\b[^}]*\}\s*from\s*['"]\.\/_lib\/qss-worlds\.js['"]/.test(srcNoComments),
    'qss-stories.js must import sanitizeSlug from ./_lib/qss-worlds.js');
});
...
if (fails.length) {
  console.error(`\nqss-stories-world-routing: ${fails.length} FAILED`);
  for (const f of fails) console.error('  ✗ ' + f);
  process.exit(1);
}
console.log(`qss-stories-world-routing: ${pass}/${pass} passed`);
