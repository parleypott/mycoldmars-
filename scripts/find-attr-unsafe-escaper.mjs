...
//   1. a locally-defined HTML escaper that is QUOTE-UNSAFE — its body maps `<`→
//      `&lt;` (the HTML-escape signature) but contains NO `"`→`&quot;`/`&#34;`/
//      `&#x22;` mapping; AND
...
//
// WHY IT STARTS GREEN (verified 2026-07-04): every escaper in the repo that is
// used in an attribute context is already quote-safe (commentbank escapeHtml/
// escapeAttr, the QSS escHtml/esc copies, translation escHtml, research/clarify
...
// non-attribute positions, so none are flagged. `--check` trips red the moment a
// NEW quote-unsafe escaper is dropped into an attribute.
//
...
function findEscapers(src) {
  ...
  while ((m = DEF.exec(src))) {
    ...
    if (!HTML_ESCAPE_SIG.test(body)) continue; // not an HTML escaper
    // First definition wins; a real escaper is defined once per file.
    if (!out.has(name)) {
      out.set(name, { safe: QUOTE_SAFE_SIG.test(body), defLine: lineAt(src, m.index) });
    }
  }
  return out;
}
