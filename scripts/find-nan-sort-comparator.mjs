...
// ── Source selection ────────────────────────────────────────────────────────
// Real authored source only. Skip tests (a gate that flagged its own fixtures
// would be useless), minified files, vendored bundles, node_modules. Same dir
// posture as find-bool-sort-comparator, plus the inline-HTML tools that carry
// recency sorts (commentbank, prawn, todo, research, nile, growth).
const SCAN_DIRS = [
...
// embed intentional `new Date(...)` comparators that would otherwise self-trip it.
const SKIP = /(\.test\.|\.spec\.|\.min\.|node_modules|\/assets\/index-|\bdist\b|find-nan-sort-comparator\.mjs)/;
...
// Pull the full parenthesised argument of every `.sort(` call so nested calls
// like `Math.abs(...)` / `new Date(...)` stay intact.
